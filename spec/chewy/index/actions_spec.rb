...
describe Chewy::Index::Actions do
  ...
  describe '.needs_migration?' do
    ...
    specify { expect(DummiesIndex.needs_migration?).to eq(true) }

    context 'nothing changed' do
      before { DummiesIndex.create }
      specify { expect(DummiesIndex.needs_migration?).to eq(false) }
    end

    context 'mappings changed' do
      before do
        DummiesIndex.create

        DummiesIndex.class_eval do
          define_type -> { [] }, name: :other_dummy do
            root id: -> { self } do
              field :name, value: -> { self }
            end
          end
